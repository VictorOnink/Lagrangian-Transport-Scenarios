...
def SizeTransport_SeparationDistance(scenario, figure_direc, size_selection, rho_selection, tau_selection, size_list,
    ...
    # Creating the figure
    ax_range = datetime(settings.STARTYEAR + 1, 1, 1), datetime(settings.STARTYEAR, 1, 1), 1000, 1
    ...
    ax[0].set_title('(a) Mean separation to r = {:.3f} mm'.format(size_selection * 1e3))
    ax[1].set_title('(b) Median separation to r = {:.3f} mm'.format(size_selection * 1e3))

    # Saving the figure
    file_name = output_direc + 'Separation_distance_size={:.3f}.png'.format(size_selection * 1e3)
