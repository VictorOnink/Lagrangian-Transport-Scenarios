...
class Stochastic(base_scenario.BaseScenario):
    ...
    def _file_names(self, new: bool = False):
        odirec = self.input_dir + "st_" + str(settings.SHORE_TIME) + "_rs_" + str(settings.RESUS_TIME) + \
        ...
        if new == True:
            ...
            return odirec + self.prefix + "_st=" + str(settings.SHORE_TIME) + "_rt=" + str(settings.RESUS_TIME) + \
                   "_y=" + str(settings.START_YEAR) + "_I=" + str(settings.INPUT) + "_r=" + str(settings.RESTART) + \
                   "_run=" + str(settings.RUN) + ".nc"
        else:
            os.system('echo "Set the restart file name"')
            return odirec + self.prefix + "_st=" + str(settings.SHORE_TIME) + "_rt=" + str(settings.RESUS_TIME) + \
                   "_y=" + str(settings.START_YEAR) + "_I=" + str(settings.INPUT) + "_r=" + str(settings.RESTART - 1) + \
                   "_run=" + str(settings.RUN) + ".nc"
