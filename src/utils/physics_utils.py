...
def _floating_AdvectionRK4DiffusionEM_stokes_depth(particle, fieldset, time):
    ...
    if particle.beach == 0:
        ...
        z_correc = d - 1.472102  # correction since the surface in the CMEMS Mediterranean data is not at 0
        particle.rise_velocity = z_correc#math.exp(-2 * k_p * z_correc) - math.sqrt(2 * math.pi * k_p * z_correc) * math.erfc(2 * k_p * z_correc)
